...
class GitOpsManager:
    ...
    def __init__(self):
        ...
        self.base_branch = "main"  # SRE: Fallback padrão dinâmico
    ...
    def _executar(self, cmd: list, cwd: str = None) -> tuple[int, str, str]:
        """SRE: Executa comandos permitindo isolamento de diretório de trabalho."""
        try:
            env = os.environ.copy()
    ...
    def _detectar_branch_padrao(self, repo_path: str = None) -> str:
        """SRE: Detecta dinamicamente se o repositório utiliza 'main' ou 'master' como raiz."""
        _, ramificacoes, _ = self._executar(["git", "branch", "-r"], cwd=repo_path)
        if "origin/main" in ramificacoes:
            return "main"
        if "origin/master" in ramificacoes:
            return "master"
        
        # Fallback local caso o remoto não responda
        _, local_active, _ = self._executar(["git", "rev-parse", "--abbrev-ref", "HEAD"], cwd=repo_path)
        active_clean = local_active.strip()
        return active_clean if active_clean else "main"
    ...
    def criar_branch_e_push(self, arquivos: list, prompt: str, repo_path: str = None) -> tuple[bool, str, str]:
        ...
        # Auto-detecta se o repositório alvo usa 'master' ou 'main' antes de começar
        self.base_branch = self._detectar_branch_padrao(repo_path)
        self.logger.info(f"GITOPS | Branch base detectada de forma autônoma: '{self.base_branch}'")
        ...
        if ret != 0 and "nothing to commit" not in out:
            self._executar(["git", "checkout", self.base_branch], cwd=repo_path)
        ...
        ret_p, out_p, err_p = self._executar(["git", "push", authenticated_url, branch_name], cwd=repo_path)

        if ret_p != 0:
            self._executar(["git", "checkout", self.base_branch], cwd=repo_path)  # Recuo preventivo por segurança
    ...
    def criar_pull_request(self, branch_name: str, prompt: str) -> tuple[bool, str]:
        ...
        payload = {
            ...
            "head": branch_name,
            "base": self.base_branch  # <<< SRE: Destino dinâmico integrado para master ou main
        }
