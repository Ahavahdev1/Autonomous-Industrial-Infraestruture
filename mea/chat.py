...
class MEA:
    ...
    def __init__(self, client, app=None, model="gpt-4o-mini"):
        ...
        # MEMÓRIA VOLÁTIL DE SESSÃO: Impede que os índices de bounties fiquem travados ou vazios
        self.cache_bounties = []
        self.guardian = GuardianEngine()
        self.logger = logging.getLogger("MEA.Kernel")
        
        # Mapeamento de intenções físicas para execução (Normalização de Verbos)
        self.cmd_map = {
            "read": "read", "ler": "read", "visualizar": "read", "read_file": "read",
            "write": "write", "criar": "write", "crie": "write", "gravar": "write", "write_file": "write",
            "run": "executar", "rodar": "executar", "executar": "executar"
        }
    ...
    def registrar_rotas(self):
        ...
        directory_name = "frontend" if os.path.exists("frontend") else "front"
    ...
    async def _handle_ataque(self, args, prompt) -> str:
        ...
        # 4. Higienização e Filtro do test_cmd (Evita rodar frases de instrução como comandos!)
        extracted_cmd = args[1] if len(args) > 1 else None
        if extracted_cmd:
            cmd_lower = str(extracted_cmd).lower()
            # Se contiver verbos de instrução em português/inglês ou for uma frase longa, descartamos do comando de teste
            instrucao_keywords = [
                "corrija", "refatore", "adicione", "implemente", "crie", 
                "missão", "missao", "mude", "altere", "delete", "remova"
            ]
            if any(k in cmd_lower for k in instrucao_keywords) or " " in cmd_lower:
                extracted_cmd = None

        # Definição do test_cmd dinâmico poliglota baseado no alvo curado
        if ext == ".py":
            test_cmd = extracted_cmd if extracted_cmd else f'"{sys.executable}" -m py_compile "{abs_target}"'
        elif ext == ".ex":
            # Usa 'elixirc' se especificado pelo usuário, senão cai no bypass de sucesso para Elixir
            test_cmd = extracted_cmd if extracted_cmd else "cmd /c echo Sucesso"
        else:
            # Para JavaScript (.js, .jsx), HTML, CSS, o teste padrão é sucesso imediato (echo)
            # Isso impede o loop de tentar rodar a frase do usuário ou py_compile em JS!
            test_cmd = extracted_cmd if extracted_cmd else "cmd /c echo Sucesso"
    ...
    async def _handle_os_action(self, args) -> str:
        """Músculos de execução física com auditoria rigorosa e auto-cura Mt 5:8."""
        try:
            acao = str(args[0]).lower().replace("funcao_", "").strip()
            # 1. Normalização de Verbos via self.cmd_map (Bypass de erros de digitação!)
            acao = self.cmd_map.get(acao, acao)
            
            target = str(args[1]).replace("arquivo ", "").strip() if len(args) > 1 else ""
            payload = args[2] if len(args) > 2 else ""

            # 2. AUTO-CURA DE CAMINHO (Mt 5:8) PARA LER/ESCREVER
            # Evita o erro de 'caminho inexistente' ao ler arquivos em subpastas!
            from mea.Mt5_8 import ConsensoSoberano
            consenso = ConsensoSoberano()
            _, caminho_curado = consenso.verificar_e_curar(True, True, target)
            target = caminho_curado
            ...
            # 4. LÓGICA DE ESCRITA (Guardian + Logos)
            if acao == "write":
            ...
            if acao == "executar":
                if not os.path.exists(target): return f"Erro: '{target}' não encontrado."
